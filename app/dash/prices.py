...
unelco_rates = pd.read_csv("app/data/electricity.csv")
...
def build_figure_one(tariff: str = "base_rate"):
    ...
    figure.add_trace(
        go.Bar(
            ...
            marker_color=current_app.config["SOURCE_COLORS"][3],
        ),
        secondary_y=False,
    )
    figure.add_trace(
        go.Line(
            ...
            name="Tariff Rate",
            marker_color=current_app.config["SOURCE_COLORS"][0],
        ),
        secondary_y=True,
    )
...
def build_figure_two(tariff: str = "base_rate"):
    ...
    figure.update_traces(marker_color=current_app.config["SOURCE_COLORS"][0])
    return figure
...
def init_dash(server):
    ...
    dash_app = Dash(server=server, routes_pathname_prefix="/energy-prices/")
