...
df = pd.read_csv("app/data/ura-market-snapshots.csv", thousands=",")
...
def build_line_chart(location: str = "Vanuatu"):
    ...
    for source, color in zip(current_app.config["SOURCE_LABELS"], current_app.config["SOURCE_COLORS"]):
...
def build_pie_chart(location: str, date):
    ...
    if location != "Vanuatu":
        data = data.query(f"location=='{location}'")
    values = [data.query(f"source=='{s}'")["kwh"].sum() for s in SOURCE_current_app.config["LABELS"]]
    figure = go.Figure(
        data=[
            go.Pie(
                labels=current_app.config["SOURCE_LABELS"],
                values=values,
                marker_colors=current_app.config["SOURCE_COLORS"],
                pull=[0.2, 0, 0, 0, 0],
            )
        ]
    )
...
def build_renewable_percent_chart(location: str = "Vanuatu"):
    ...
    figure.for_each_trace(lambda trace: trace.update(fillcolor = current_app.config["SOURCE_COLORS"][1]))
    figure.for_each_trace(lambda trace: trace.line.update(color = current_app.config["SOURCE_COLORS"][2]))
    return figure
...
def init_dash(server):
    ...
    dash_app = Dash(server=server, routes_pathname_prefix="/energy-sources/")
